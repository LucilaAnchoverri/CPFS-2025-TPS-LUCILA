...
async addProductoToFactura(nroFactura:number,idProducto:number,cantidad:number):Promise<void>{
  ...
  async update(nroFactura: number, dto: FacturaDto): Promise<Factura> {
    ...
    // Actualizamos la factura base
    facturaExistente.fecha = dto.fecha;
    facturaExistente.total_sin_iva = dto.total_sin_iva;
    facturaExistente.iva = dto.iva;
    facturaExistente.total_con_iva = dto.total_con_iva;
    facturaExistente.cliente = cliente;

    const facturaActualizada = await this.facturaRepository.save(facturaExistente);

    // Limpiar tabla intermedia y reinsertar detalle
    await this.dataSource.query(
      `DELETE FROM E01_DETALLE_FACTURA WHERE nro_factura = ?`,
      [nroFactura]
    );
    ...
    for (let i = 0; i < dto.productos.length; i++) {
      await this.dataSource.query(
        `INSERT INTO E01_DETALLE_FACTURA (nro_factura, codigo_producto, nro_item, cantidad) VALUES (?, ?, ?, ?)`,
        [nroFactura, dto.productos[i], i + 1, dto.cantidad[i]]
      );
    }
