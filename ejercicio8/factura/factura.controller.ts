...
export class FacturaController {
    ...
    @Put(':nroFactura')
    async update(
    @Param('nroFactura', ParseIntPipe) nroFactura: number,@Body(new ValidationPipe({ transform: true })) facturaDto: FacturaDto): Promise<Factura> {
        try {
            return await this.facturaService.update(nroFactura, facturaDto);
        } catch (error) {
